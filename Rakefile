...
def setup_globals
  ...
  $programs                =  %w{mkvmerge mkvcut mkvinfo mkvextract mkvpropedit}
  ...
  $source_directories      =  %w{lib/avilib-0.6.10 lib/librmff src src/input src/output src/common src/common/chapters src/common/compression src/common/strings src/common/tags src/common/xml
                                 src/mmg src/mmg/header_editor src/mmg/options src/mmg/tabs src/extract src/propedit src/merge src/cut src/info src/mpegparser}
...
namespace :apps do
  task :cli => %w{apps:mkvmerge apps:mkvcut apps:mkvinfo apps:mkvextract apps:mkvpropedit}
...
namespace :install do
  ...
  task :shared do
    ...
    wanted_apps     = %w{mkvmerge mkvcut mkvmergeGUI mkvinfo mkvextract mkvpropedit}.collect { |e| "#{e}.png" }.to_hash_by
...
[ { :name => 'avi',         :dir => 'lib/avilib-0.6.10'                                                                             },
  ...
  { :name => 'mtxcut',      :dir => 'src/cut',      :except => [ 'mkvcut.cpp' ],                                                    },
...
Application.new("src/mkvmerge").
  ...
  libraries(:mtxmerge, :mtxinput, :mtxoutput, $common_libs, :avi, :rmff, :mpegparser, :flac, :vorbis, :ogg, $custom_libs).
  create
...
Application.new("src/mkvcut").
  description("Build the mkvcut executable").
  aliases(:mkvcut).
  sources("src/cut/mkvcut.cpp").
  sources("src/cut/resources.o", :if => c?(:MINGW)).
  libraries(:mtxcut, :mtxinput, :mtxoutput, $common_libs, :avi, :rmff, :mpegparser, :flac, :vorbis, :ogg, $custom_libs).
  create
