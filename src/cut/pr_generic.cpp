    ...
    if (track_audio == m_htrack_type) {
      ...
      mxwarn_tid(m_ti.m_fname, m_ti.m_id,
                 boost::format(Y("The current packet's timecode is smaller than that of the previous packet. "
                                 ...
                                 "This may throw audio/video synchronization off, but that can be corrected with mkvcut's \"--sync\" option. "
                                 ...
                                 "then either is the source file badly mastered, or mkvcut contains a bug. "
...
generic_packetizer_c::show_experimental_status_version(std::string const &codec_id) {
  ...
  mxwarn(boost::format(Y("Note that the Matroska specifications regarding the storage of '%1%' have not been finalized yet. "
                         "mkvcut's support for it is therefore subject to change and uses the CodecID '%2%/EXPERIMENTAL' instead of '%2%'. "
                         "This warning will be removed once the specifications have been finalized and mkvcut has been updated accordingly.\n"))
