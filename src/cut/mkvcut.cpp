...
set_usage() {
  usage_text  =   "";
  usage_text += Y("mkvcut -o out [global options] [options1] <file1> [@optionsfile ...]\n");
  usage_text +=   "\n";
  ...
  usage_text += Y("  --append-mode <file|track>\n"
                  "                           Selects how mkvcut calculates timecodes when\n"
                  "                           appending files.\n");
  ...
  usage_text += Y("  --list-languages         Lists all ISO639 languages and their\n"
                  "                           ISO639-2 codes.\n");
  usage_text += Y("  --capabilities           Lists optional features mkvcut was compiled with.\n");
  usage_text += Y("  --priority <priority>    Set the priority mkvcut runs with.\n");
...
#endif
  usage_text +=   "\n\n";
  usage_text += Y("Please read the man page/the HTML documentation to mkvcut. It\n"
                  "explains several details in great length which are not obvious from\n"
                  "this listing.\n");

  version_info = get_version_info("mkvcut", vif_full);
}

/** \brief Prints information about what has been compiled into mkvcut
*/
static void
print_capabilities() {
...
identify(std::string &filename) {
  ...
  if (FILE_TYPE_IS_UNKNOWN == file.type)
    mxerror(boost::format(Y("File %1% has unknown type. Please have a look at the supported file types ('mkvcut --list-types') and "
  ...
  if (check) {
    ...
    if (-1 == index)
      mxerror(boost::format(Y("'%1%' is neither a valid ISO639-2 nor a valid ISO639-1 code. "
                              "See 'mkvcut --list-languages' for a list of all languages and their respective ISO639-2 codes.\n")) % parts[1]);
...
handle_file_name_arg(const std::string &this_arg,
  ...
  for (auto &file_name : file_names) {
    if (file_name.empty())
      ...
      mxerror(boost::format(Y("The name of the output file '%1%' and of one of the input files is the same. This would cause mkvcut to overwrite "
  ...
  if (FILE_TYPE_IS_UNKNOWN == file.type)
    mxerror(boost::format(Y("The file '%1%' has unknown type. Please have a look at the supported file types ('mkvcut --list-types') and "
                            "contact the author Moritz Bunkus <moritz@bunkus.org> if your file type is supported but not recognized properly.\n")) % file.name);
...
parse_args(std::vector<std::string> args) {
  ...
  mxinfo(boost::format("%1%\n") % get_version_info("mkvcut", vif_full));
  ...
  mxforeach(sit, args) {
    ...
    if ((this_arg == "--priority")) {
      ...
      mxwarn(Y("The option '--meta-seek-size' is no longer supported. Please read mkvcut's documentation, especially the section about the MATROSKA FILE LAYOUT.\n"));
      sit++;
