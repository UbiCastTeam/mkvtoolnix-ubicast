...
/** \brief Fix the file after mkvcut has been interrupted

   On Unix like systems mkvcut will install a signal handler. On \c SIGUSR1
   all debug information will be dumped to \c stdout if mkvcut has been
   compiled with \c -DDEBUG.

   On \c SIGINT mkvcut will try to sanitize the current output file
...
sighandler(int /* signum */) {
  if (!s_out)
    mxerror(Y("mkvcut was interrupted by a SIGINT (Ctrl+C?)\n"));

  mxwarn(Y("\nmkvcut received a SIGINT (probably because the user pressed "
           "Ctrl+C). Trying to sanitize the file. If mkvcut hangs during "
  ...
  try {
    ...
    if (s_muxing_app.empty()) {
      if (!hack_engaged(ENGAGE_NO_VARIABLE_DATA)) {
        ...
        s_writing_app  = get_version_info("mkvcut", static_cast<version_info_flags_e>(vif_full | vif_untranslated));
  ...
  for (auto &src_file : g_files) {
    ...
    if ((0 < count) && (src_file.reader->m_used_track_ids.size() > count))
      ...
      mxinfo(boost::format(Y("No append mapping was given for the file no. %1% ('%2%'). A default mapping of %3% will be used instead. "
                             "Please keep that in mind if mkvcut aborts with an error message regarding invalid '--append-to' options.\n"))
